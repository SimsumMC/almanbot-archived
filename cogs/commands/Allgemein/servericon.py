...
class servericon(commands.Cog):
    ...
    async def servericon(self,  ctx):
        ...
        if name == botchannel or botchannel == 'None':
            embed = discord.Embed(title=f"**Servericon von {ctx.guild.name}**", colour=get_colour(ctx.message))
            ...
            log(f'{time}: Der Spieler {user} hat den Befehl {get_prefix_string(ctx.message)}'
                'servericon benutzt!', id=ctx.guild.id)
        else:
            log(input=f'{time}: Der Spieler {user} hat probiert den Befehl {get_prefix_string(ctx.message)}'
                      f'servericon im Channel #{name} zu benutzen!', id=ctx.guild.id)
