...
class resume(commands.Cog):
    ...
    async def resume(self, ctx):
        ...
        if not ctx.author.voice:
            ...
            await log(
                f"{time}: Der Nutzer {user} hat versucht den Befehl {await get_prefix_string(ctx.message)}"
                "resume zu benutzen, befand sich aber nicht in dem gleichen Sprachkanal wie der Bot!",
                guildid=ctx.guild.id,
            )
            return
            ...
            embed = discord.Embed(
                title="Fehler",
                description="Du musst erst die Musik pausieren!",
                colour=await get_embedcolour(ctx.message),
            )
            embed._footer = await get_embed_footer(ctx)
            embed._thumbnail = await get_embed_thumbnail()
            ...
            await log(
                f"{time}: Der Nutzer {user} hat versucht den Befehl {await get_prefix_string(ctx.message)}"
                "resume zu benutzen, es wurde aber kein Song pausiert!",
                guildid=ctx.guild.id,
            )
            return
            ...
            await log(
                f"{time}: Der Nutzer {user} hat versucht den Befehl {await get_prefix_string(ctx.message)}"
                "resume zu benutzen, es wurde aber kein Song abgespielt!",
                guildid=ctx.guild.id,
            )
            return
